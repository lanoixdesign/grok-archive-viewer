...
let rootHandle = null; // Pour garder l'accès en écriture au dossier racine

const jsonLinksEl = document.getElementById('jsonLinks');
const localFilesEl = document.getElementById('localFiles');

// Helpers
function setLoading(isLoading) { document.getElementById('loadingStatus').style.display = isLoading ? 'block' : 'none'; }
function toggleAccordion(id, el) { document.getElementById(id).classList.toggle('hidden'); el.classList.toggle('collapsed'); }
...
document.getElementById('loadArchiveBtn').onclick = async () => {
    try {
        // On demande l'accès en lecture/écriture dès le début
                        ...
                        for await (const assetFile of subFolder.values()) {
                            if (assetFile.name.startsWith('content')) {
                                ...
                                allAssets.push({
                                    ...
                                    // Sauvegarde des handles pour la suppression ultérieure
        ...
        renderGallery();
        alert("Archive synchronisée avec succès !");
    } catch (e) { 
        console.error(e);
...
// ==========================================
// SUPPRESSION GROUPÉE (Seulement sélection)
// ==========================================
async function deleteSelected() {
    ...
    if (toDelete.length === 0) return alert("Veuillez d'abord cocher des fichiers locaux à supprimer.");
    
    if (!confirm(`🚨 SUPPRESSION DÉFINITIVE\n\nVoulez-vous vraiment supprimer ces ${toDelete.length} dossiers de votre ordinateur ?\nCette action est irréversible.`)) return;

    setLoading(true);
    let successCount = 0;

    try {
        // On vérifie la permission une seule fois pour tout le lot
        ...
        for (const asset of toDelete) {
            try {
                ...
                console.error(`Erreur pour ${asset.id}:`, err);
            }
        }
        ...
        renderGallery();
        alert(`${successCount} dossiers supprimés avec succès.`);
    } catch (err) {
        alert("La suppression a échoué. Vérifiez les permissions du dossier.");
    } finally {
        setLoading(false);
    }
}
    ...
    currentGalleryItems.forEach((asset) => {
        ...
        if (asset.source === 'JSON') {
            ...
            item.innerHTML = `
                ...
                <input type="checkbox" data-url="${asset.url}" onclick="event.stopPropagation()" style="margin-left:10px">
            `;
            item.onclick = () => { 
            ...
            card.innerHTML = `
                ...
                <div class="info">
                    ...
                    <label style="cursor:pointer; font-size:0.8rem; display:flex; align-items:center; gap:8px; margin-top:5px;">
...
window.openLightbox = function(i) {
    ...
    const targetEl = document.querySelector(`[data-url="${a.url}"]`)?.closest('.local-card');
    markAsViewed(targetEl);
};
...
window.openProfileModal = function() {
    if(!authData) return alert("Chargez votre archive d'abord.");
